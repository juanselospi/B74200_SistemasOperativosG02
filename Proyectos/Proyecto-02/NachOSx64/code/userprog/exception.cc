...
static void ReadUserString(int userAddr, char *buf, int maxSize) { // evita repetir lo mismo en varias syscalls
   int i = 0;
   int val = 0;
   ...
   // RECORDATORIO: revisar caso cuando el string no termina en \0
   buf[i] = '\0'; 
}
...
static void MarkProcessExited(int status) {
   ...
   for(int i = 0; i < MAX_PROCESSES; i++) {
      ...
      if(processMap->Test(i) && processTable[i] != NULL && processTable[i]->thread == currentThread) {
         // revisar de nuevo la variable
         processTable[i]->exitStatus = status;
         processTable[i]->exited = true;
         processTable[i]->joinSem->V(); // ver codigo de paralela con chiki
...
static void NachosForkThread(void *arg) {
   // cout<< "LLAMdADO A NACHOSFORKTHREAD";
...
void NachOS_Open() {
   ...
   // fix cuando no se puede abrir el file
   returnFromSystemCall();
}
...
void NachOS_Fork() {
   ...
   Thread *newT = new Thread("child para ejecutar fork code");
...
void NachOS_SemWait() {
   ...
   if(semMap == NULL || id < 0 || !semMap->Test(id) || userSems[id] == NULL) { // probar con verificacion para <0
...
void NachOS_LockDestroy() {
   ...
   if(lockMap == NULL || id < 0 || !lockMap->Test(id) || userLocks[id] == NULL) { // buscar cambios a <0 en las demas
...
void NachOS_CondSignal() {
   ...
   if(condMap == NULL || lockMap == NULL || condId < 0 || !condMap->Test(condId) || userConds[condId] == NULL || lockId < 0 || !lockMap->Test(lockId) || userLocks[lockId] == NULL) {
